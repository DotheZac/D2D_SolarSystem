...
void TransformPracticeScene::Tick(float deltaTime)
{
   //입력 이벤트처리
    ...
    globalRenderer.RenderBegin();           //beginDraw, 흰색으로 초기화
    globalRenderer.SetTransform(finalTM);   //d2dcontext를 finaltm으로
...
void TransformPracticeScene::AddBoxObjects(D2D1_POINT_2F point, int i_image)
{
    ...
    BoxObject* pNewBox = new BoxObject(m_BitmapPtr[i_image]);        //이미지 집어넣고

    pNewBox->SetPosition(Vec2(worldPt.x, worldPt.y));       //카메라 역행렬한 위치를 박스 위치로 넣고

    pNewBox->ToggleSelfRotation();

    m_BoxObjects.push_back(pNewBox);                        //오브젝트 컨테이너에 집어넣기
...
void TransformPracticeScene::UpdateRelationship()
{
    ...
    if (it == m_SelectedBoxObjects.end()) return; //박스 없으면 종료

    (*it)->SetLeader(true);

    if (m_SelectedBoxObjects.size() < 2) return; //두번째 박스가 없으면 종료

    
    while (it != m_SelectedBoxObjects.end() && std::next(it) != m_SelectedBoxObjects.end())
        //현재 이터레이터가 리스트의 끝도 아니고 그 다음도 끝이 아니면
    {
        BoxObject* parent = *it;
